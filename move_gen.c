...
void SortOrdinary(const typeMoveList* m1, typeMoveList* m2, const uint32_t s1, const uint32_t s2, const uint32_t s3)
{
	typeMoveList *p, *q;
...
typeMoveList* EvasionMoves(const typePos* Position, typeMoveList* list, const uint64_t mask)
{
	if (Position->wtm)
...
typeMoveList* OrdinaryMoves(const typePos* Position, typeMoveList* list)
{
	if (Position->wtm)
...
typeMoveList* CaptureMoves(const typePos* Position, typeMoveList* list, const uint64_t mask)
{
	if (Position->wtm)
...
typeMoveList*MyEvasion(const typePos* Position, typeMoveList* List, uint64_t c2)
{
	uint64_t U;
	int to, fr, c;
	const int king = MyKingSq;
	uint64_t att = MyKingCheck;
	int sq = LSB(att);
	int pi = Position->sq[sq];
	uint64_t mask = ~OppAttacked & ((pi == EnumOppP ? AttK[king] : 0) | Evade[king][sq]) & ~MyOccupied & c2;
	BitClear(sq, att);
	...
	uint64_t T = BitboardMyP & BackShift((c2 &OppOccupied) ^ c2);
...
typeMoveList*MyPositionalGain(const typePos* Position, typeMoveList* List, const int av)
{
	const uint64_t empty = ~Position->OccupiedBW;
	uint64_t U, T;
	int to, sq;
	typeMoveList *p, *q;
	int move;
	const typeMoveList* sm = List;
...
typeMoveList*MyCapture(const typePos* Position, typeMoveList* List, const uint64_t mask)
{
	uint64_t U, AttR, AttB;
	int sq, c;
	int to = Position->Current->ep;
	...
	uint64_t T = CaptureLeft & ~BitboardEighthRank & mask;
...
typeMoveList*MyOrdinary(const typePos* Position, typeMoveList* List)
{
	const uint64_t empty = ~Position->OccupiedBW;
	uint64_t U, T, ROOK, BISHOP;
	int to, sq;
	const int requ = OppKingSq;
	...
	const uint64_t Pawn = MyAttackedPawns[requ];
