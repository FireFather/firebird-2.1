...
static int WhiteKingDanger(const typePos* Position, const int wKs)
{
	const int rank = RANK(wKs);
	const uint64_t A = wBitboardP & NotInFrontB[rank];
	const typePawnPtr Z = PawnPtr[FILE(wKs)];
	uint64_t T = A & Z.Edge;
	int RankWa = LSB(T);
	...
	if (!T)
		RankBc = 0;
	RankBc >>= 3;
	int v = Z.ShelterEdge[RankWa] + Z.ShelterMiddle[RankWb] + Z.ShelterCenter[RankWc];
...
static int BlackKingDanger(const typePos* Position, const int bKs)
{
	const int rank = RANK(bKs);
	const uint64_t A = bBitboardP & NotInFrontW[rank];
	const typePawnPtr Z = PawnPtr[FILE(bKs)];
	uint64_t T = A & Z.Edge;
	int RankBa = MSB(T);
	...
	RankWc >>= 3;
	RankWc = 7 - RankWc;
	int v = Z.ShelterEdge[RankBa] + Z.ShelterMiddle[RankBb] + Z.ShelterCenter[RankBc];
...
void PawnEval(typePos* Position, typePawnEval* RESULT)
{
	...
	Ptr = PawnHash + (Position->Current->PawnHash & (uint64_t)PawnHashSize - 1);
