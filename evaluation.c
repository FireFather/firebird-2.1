...
static void AdjustPositionalGainB(const typePos* Position, const int move)
{
	if (Position->Current->cp)
		return;

	const int p = Position->sq[To(move)];
	const int m = move & 07777;
	const int v = Position->Current->PositionalValue - (Position->Current - 1)->PositionalValue;
...
static void AdjustPositionalGainW(const typePos* Position, const int move)
{
	if (Position->Current->cp)
		return;
	...
	const int p = Position->sq[To(move)];
	const int m = move & 07777;
	const int v = (Position->Current - 1)->PositionalValue - Position->Current->PositionalValue;
...
{
	for (int c = 0; c < EvalHashSize; c++)
		EvalHash[c] = 0;
}

static int MaterialValue(const typePos* Position)
{
...
static void KingPawnWhite(typePos* Position, const int matval, const uint8_t Token, typePawnEval* PawnInfo)
{
	int sq, rank;
	...
	int Value = Position->Current->PST + PawnInfo->score;
	Value = (int16_t)(Value & 0xffff);
	int WhiteLeader = 0;
	uint8_t C = PawnInfo->wPassedFiles;
	...
	uint64_t A = (wBitboardP & ~FileA) << 7;
	uint64_t T = A & bBitboardK;
	...
	if (Position->Current->Value > 0)
	{
		...
		if ((Position->Current->flags & 28) == 28)
		{
			...
			if (!Value)
				...
				Position->Current->Value = (int16_t)(Position->Current->PST & 0xffff) + 75 * rank + 250;
		}
	}
...
static void KingPawnBlack(typePos* Position, const int matval, const uint8_t Token, typePawnEval* PawnInfo)
{
	int sq, rank;
...
void Eval(typePos* Position, int min, int max, int move)
{
	...
	PawnPointer = PawnHash + (Position->Current->PawnHash & (uint64_t)PawnHashSize - 1);
	PrefetchPawnHash
	...
	{
		Value = (int)(int16_t)(EvalHash[Position->Current->Hash & EvalHashMask] & 0xffff);
	...
	{
		Position->wtm
			? KingPawnWhite(Position, matval, Token, PawnInfo)
			: KingPawnBlack(Position, matval, Token, PawnInfo);
