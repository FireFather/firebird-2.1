...
#include "bitops.h"
#include <stdbool.h>
static INLINE void SMPBadHistory(const typePos* pos, const uint32_t m, const SplitPoint* sp)
{
...
void MyPVNodeSMP(typePos* Position)
{
	...
	while (true)
	{
		...
		const int m = MyNext(Position, NextMove);
		...
		{
			const boolean b = OppKingCheck != 0;
