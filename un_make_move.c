...
static INLINE void UnMakeWhiteOO(typePos* Position, const int to)
{
	if (to == G1)
	{
		wBitboardOcc ^= F1H1;
...
static INLINE void UnMakeBlackOO(typePos* Position, const int to)
{
	if (to == G8)
	{
		bBitboardOcc ^= F8H8;
...
void UndoWhite(typePos* Position, const uint32_t move)
{
	const int fr = From(move);
	const int to = To(move);
	int pi = Position->sq[to];
	Position->wtm ^= 1;
	Position->height--;
	...
	wBitboardOcc &= mask;
	Position->bitboard[pi] &= mask;
	const int cp = Position->Current->cp;
...
void UndoBlack(typePos* Position, const uint32_t move)
{
	const int fr = From(move);
	const int to = To(move);
	int pi = Position->sq[to];
...
void Undo(typePos* Position, const uint32_t move)
{
	if (!Position->wtm)
