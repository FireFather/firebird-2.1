...
void CheckDone( typePos *Position, const int d )
	...
	const int64_t y = GetClock() - CLOCK_UCI;
...
void TimeManager(const int64_t Time, const int64_t Inc, int MTG)
	{
	if (MTG)
		{
		if (MTG > 25)
			MTG = 25;
		DesiredTime = Time / MTG + Inc;
		...
		if (AbsoluteTime < 1000)
			AbsoluteTime = 1000;
		DesiredTime = Time * DesiredMillis / 1000 + Inc;
		}
