...
static INLINE void MakeWhiteOO(typePos* Position, const int to)
{
	if (to == G1)
	{
		wBitboardOcc ^= F1H1;
...
static INLINE void MakeBlackOO(typePos* Position, const int to)
{
	if (to == G8)
	{
		bBitboardOcc ^= F8H8;
...
void MakeWhite(typePos* Position, const uint32_t move)
{
	Position->nodes++;
	...
	int pi = Position->sq[fr];
	Position->Current++;
	Position->Current->reversible++;
	Position->Current->move = move;
	const int fl = CastleTable[fr] & CastleTable[to] & Position->Current->oo;
...
void MakeBlack(typePos* Position, const uint32_t move)
{
	Position->nodes++;
	memcpy(Position->Current + 1, Position->Current, 32);
	const int fr = From(move);
...
void Make(typePos* Position, const uint32_t move)
{
	if (Position->wtm)
	{
		if (NodeCheck & 4095)
