...
void EndSMP()
{
	...
	if (active_threads > 1)
	{
		const int cause = (int)WaitForSingleObject(Wakeup[0], 1000);
...
static void SMPGoodHistory(const typePos* Pos, const uint32_t m, const SplitPoint* sp)
{
	const int sv = HistoryValue(Pos, m);
	HistoryValue(Pos, m) = sv + ((0xff00 - sv) * sp->depth >> 8);
}

void SMPFailHigh(SplitPoint* sp, const typePos* Pos, const uint32_t m)
{
	Lock(sp->splock);
