...
int MyAll(typePos* Position, const int VALUE, const int depth)
{
	...
	const uint64_t zob = Position->Current->Hash;
	...
	while (true)
	{
			...
			&& NextMove->phase <= Ordinary_Moves)
		{
			int r;
			Split = true;
			const boolean b = SMPSplit(Position, NextMove, depth, VALUE, VALUE, NodeTypeAll, &r);
			CheckHalt()
		...
		else
		{
			...
			{
				new_depth = depth - 2 + Extend - MSB((uint64_t)1 + cnt);
